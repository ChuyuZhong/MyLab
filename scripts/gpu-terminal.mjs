...
function get(q,s){terminalId(q.id);const t=sessions.get(q.id);if(!t||t.owner!==owner(s))throw Error('终端已断开，请重新连接');t.touched=Date.now();return t;}
export async function openTerminal(q,s,getShell){
 ...
 if(sessions.has(q.id)){const t=get(q,s);if(t.taskId!==q.taskId)throw Error('终端任务不匹配');return {id:q.id};}
 ...
 const promise=(async()=>{
  ...
  const t={proc,owner:owner(s),taskId:q.taskId,text:'',offset:0,seq:0,closed:false,touched:Date.now()};sessions.set(q.id,t);
  proc.onData(chunk=>{t.text+=chunk;if(t.text.length>262144){const n=t.text.length-262144;t.text=t.text.slice(n);t.offset+=n;}});
  proc.onExit(()=>{t.closed=true;});return {id:q.id};
 })();opening.set(q.id,promise);try{return await promise;}finally{opening.delete(q.id);}
}
export function terminalAction(action,q,s){const t=get(q,s);
